...
imageName: "what-the-phd-gave-me/phd1.jpeg"
...
My qualifying exam was on object detection and tracking. I had assumed that was where I'd stay. What actually happened is that the first year of the PhD was mostly me saying yes to whatever my labmates were doing. I helped a senior student run experiments. I joined a team for a remote sensing competition we didn't expect to win, and we didn't. But to compete we had to take the [Vision Transformer](https://arxiv.org/abs/2010.11929) apart in detail, and that detail turned into the seed of the first paper I published the next year.
...
For me that meant building a map. When I started working on masked image modeling I went through every paper that cited [MAE](https://arxiv.org/abs/2111.06377) and [BEiT](https://arxiv.org/abs/2106.08254). First pass was abstracts and results tables only. I kept a running list with paper names, datasets, experiments, and reported numbers. The common datasets surfaced quickly: [ImageNet](https://www.image-net.org/) for linear probe and finetune, [ADE20K](https://paperswithcode.com/dataset/ade20k) for semantic segmentation, [COCO](https://cocodataset.org/) for object detection. For the strongest results I sketched the architectures on a whiteboard and took photos so I could put them side by side later. The differences started to cluster: RGB-based, codebook-based, EMA self-distillation, teacher-based like [CLIP](https://arxiv.org/abs/2103.00020). I read the ablations to see what part of each model was actually doing the work. By the end I had a real mental model of what had been tried, what worked, and where there might be room to add something.
...
After that year my own work shifted. I started treating GitHub like the long version of my CV. I kept repos private during review, then made them open the day each paper landed. I uploaded weights to [HuggingFace](https://huggingface.co/). I built small demos when I could. I kept the code reproducible enough that I'd be willing to walk a reviewer through any line of it. The selfish reasons are real: clean code makes your follow-up papers easier and your own re-runs less painful six months later. The recruiting angle is real too. But the underlying reason is the bigger one. After a year of wishing somebody else's code was open, the least I could do was return the favor.

If you're about to pick a SOTA baseline, the rule I now follow: reproduce their published numbers first, before you touch anything. Get the reproduction done in week one, not month six. If the code isn't public, or the numbers don't reproduce on the kind of cluster you actually have, pick a different baseline and don't look back. The year I spent learning this lesson the hard way is a year I don't want anyone else to spend.
