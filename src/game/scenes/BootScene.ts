...
export class BootScene extends Phaser.Scene {
  ...
  preload(): void {
    ...
    // Player spritesheet — brendan or may depending on gender choice.
    // 288x32: 18 frames of 16x32 (9 walk + 9 run).
    const isGirl = getSave().playerGender === "girl";
    this.load.spritesheet("player", `/game/sprites/emerald/${isGirl ? "may" : "brendan"}.png`, {
      frameWidth: 16,
      frameHeight: 32,
    });
    ...
    // Slaking — OG pokeemerald party menu icon (32x64, 2 frames stacked).
    // Authentic GBA pixel art; same format as every wild Pokemon icon.
    this.load.spritesheet("slaking", "/game/sprites/emerald/slaking.png", {
      frameWidth: 32,
      frameHeight: 32,
    });

    // Slakoth — OG pokeemerald party menu icon (32x64, 2 frames stacked).
    ...
    // Gate placeholder — a hot-pink 16×16 square generated inline so
    // dev-time gate definitions can reference a texture without
    // requiring an art asset. Real gate sprites override this with
    // their own `spriteKey` during the content phase. The unmistakable
    // color guarantees a placeholder can never ship unnoticed.
    //
    // `make.graphics({})` creates a Graphics object NOT added to the
    // display list (the default for `make.*` helpers), which is what
    // we want for a texture-generation scratchpad.
    const gateGfx = this.make.graphics({});
    gateGfx.fillStyle(0xff00ff, 1);
    gateGfx.fillRect(0, 0, 16, 16);
    gateGfx.generateTexture("placeholder_gate", 16, 16);
    gateGfx.destroy();
  }
